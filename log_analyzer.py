...
class AnalysisWindow(QWidget):
    ...
    def run_analysis(self):
        ...
        if not self.parsed_secs_log and not self.parsed_json_log:
            self.results_display.setText("Error: Load at least one log file first."); return
        if not self.analysis_scenario:
            self.results_display.setText("Error: Load an analysis scenario first."); return

        report = {"name": "Log Analysis", "result": "Pass", "duration": "N/A", "steps": []}
        
        # --- (Full analysis logic would go here) ---
        # For this demo, we'll just create a placeholder result.
    ...
    # ... rest of AnalysisWindow is unchanged ...
    def _setup_ui(self):
        self.secs_log_display = QTextEdit(); self.secs_log_display.setReadOnly(True); self.json_log_display = QTextEdit(); self.json_log_display.setReadOnly(True)
        self.results_display = QTextEdit(); self.results_display.setReadOnly(True); self.scenario_list = QListWidget()
        ...
        log_viewers = QHBoxLayout(); secs_pane = QVBoxLayout(); secs_pane.addWidget(QLabel("SECS/GEM Log (CSV)")); secs_pane.addWidget(self.secs_log_display)
        json_pane = QVBoxLayout(); json_pane.addWidget(QLabel("MHS Log (JSON)")); json_pane.addWidget(self.json_log_display)
        log_viewers.addLayout(secs_pane); log_viewers.addLayout(json_pane); bottom_layout = QHBoxLayout(); bottom_layout.addWidget(self.scenario_list); bottom_layout.addWidget(self.results_display)
