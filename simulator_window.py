...
from PySide6.QtWidgets import (QMainWindow, QTextEdit, QVBoxLayout, QHBoxLayout, QWidget,
                               ...
                               QAbstractItemView, QLineEdit, QListWidgetItem)
...
class SimulatorWindow(QMainWindow):
    ...
    def _setup_ui(self):
        ...
        load_action.triggered.connect(self.load_scenario_from_file)
        ...
        editor_layout.addWidget(add_step_btn)
        editor_layout.addWidget(remove_step_btn)
    ...
    def populate_step_editor(self, current, previous):
        while self.step_editor_layout.rowCount() > 0:
            self.step_editor_layout.removeRow(0)
        
        row = self.steps_list_widget.currentRow()
        if row == -1 or not current:
            return
        ...
        action_combo.addItems(["send", "expect", "if", "loop", "delay", "call", "log"])
        ...
        if action in ['send', 'expect']:
            msg_combo = QComboBox()
            msg_combo.addItems(self.factory.message_names)
            msg_combo.blockSignals(True)
            msg_combo.setCurrentText(step_data.get('message'))
            msg_combo.blockSignals(False)
            ...
            seconds_spin.setMinimum(1)
            seconds_spin.blockSignals(True)
            seconds_spin.setValue(step_data.get('seconds', 1))
    ...
    def update_list_item_text(self, row):
        ...
        if action in ["SEND", "EXPECT"]:
            details = step.get('message', '')
        # FIX: The ghost variable 'i' is replaced with the correct variable 'row'
